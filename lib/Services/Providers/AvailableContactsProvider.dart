...
//--under dev
  // searchAvailableContactsInDatabase2(
  //   BuildContext context,
  //   String currentuserphone,
  //   SharedPreferences existingPrefs,
  // ) async {
  //   List<dynamic> contactsToCheckInServer = [];
  ...
  //           contactsToCheckInServer.add(key);
  //         } else {
  ...
  //         .where(Dbkeys.phonenumbervariants, whereIn: contactsToCheckInServer)
  //         .get()
  //         .then((users) {
  //       if (users.docs.length > 0) {
  //         List<QueryDocumentSnapshot<Map<String, dynamic>>> registeredUsers =
  //             users.docs;
  //         registeredUsers.removeWhere(
  //             (user) => user.data()[Dbkeys.phone] == currentuserphone);
  //         registeredUsers.forEach((registeredUser) async {
  ...
  //               int i = joinedUserPhoneStringAsInServer
  ...
  //             if (registeredUsers.indexOf(registeredUser) ==
  ...
  //     }).catchError((onError) {
  //       Fiberchat.toast(
  //           "Cannot check the registered contacts. Please try again  \n\n$onError");
  //     });
  //   }
  // }
  ...
  searchAvailableContactsInDatabase(
    ...
    if (searchingcontactsindatabase == false &&
      ...
      searchingcontactsindatabase = false;
      notifyListeners();
      // Fiberchat.toast("searched finished");
                ...
                if (docs.docs[0].data().containsKey(Dbkeys.joinedOn)) {
                  if (joinedUserPhoneStringAsInServer.indexWhere((element) =>
                    ...
                    docs.docs[0]
                      ...
                      final String encodedavailablePhoneString =
                          JoinedUserModel.encode(joinedcontactsInSharePref);
                      if (OnlyPeerWhoAreSavedInmyContactCanMessageOrCallMe ==
                          false) {
                        await existingPrefs.setString('availablePhoneString',
                            encodedavailablePhoneString);
                      }
                    });
